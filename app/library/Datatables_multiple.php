...
class Datatables {
    ...
    public function __construct($di, $database) {
        $this->_di = $di;
        ...
        if ($search != '') {
            ...
            foreach ($columns as $column) {
                if ($column['searchable'] == "true" && $column['name'] != '') {
                    $where_or[] = " CAST ({$column['name']} AS TEXT )  ILIKE '%$search%' ";
                }
            }
            if (count($where_or) > 0) {
...
//        die($sqlCount);
        ...
        foreach ($resultados as $i => $resultado) {
            ...
            $data[$i]['actions'] = '<center><label class="radio"><input type="radio" name="selrow" class="selrow" value="' . $DT_RowId . '"  ><i></i> </label></center>';
