...
oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" 
                                                                                ...
                                                                                <td class="p_ea" style="background:#FFFDB2;" >
                                                                                    ...
                                                                                    <input type="text" style="width: 50px;text-align:center;" class="input-xs pea not get-{{ det["alumno"] }}" onkeyup="calculaFormula($(this))" name="ea[]" value="{{ det["ea"] }}" 
oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" 
                                                                                    {{ ea_activado }} disabled>
                                                                                </td>
                                                                                <td class="p_pf" style="background:#D0FFBF;" >
                                                                                    <input type="text" style="width: 50px;text-align:center;" class="input-xs ppf pf-{{ det["alumno"] }}" name="pf[]" value="{{ det["pf"] }}" readonly>
                                                                                </td>
...
<script type="text/javascript">
    ...
    {% for i in 1..20 %}
        ...
        {% if prom_conf[etq] %}
            ...
            {% endif %}
    {% endfor %}
    var formula = {{ prom_conf["formula"] }};
    console.log(formula);
</script>

<script type="text/javascript">
    function calculaFormula(t){
        ...
        //for(i=1;i<=bucle_notas;i++){
            ...
            {% for i in 1..enum_r %}
                ...
                {% if prom_conf[etq] %}
                        ...
                        var {{ prom_conf[etq] }} = parseFloat(row.find('.p_p{{ i }}').find("[data-etq='{{ prom_conf[etq] }}']").val());
                        if (isNaN({{ prom_conf[etq] }})) {
                            {{ prom_conf[etq] }} = 0;
                        }
                        console.log({{ prom_conf[etq] }});                                                            
                {% endif %}
            {% endfor %}        
        var formula = {{ prom_conf["formula"] }};    
        //}
        formula = Math.round(formula);
        row.find('.p_pf').find('.ppf').val(formula);
        console.log(formula);
    }
</script>
<script type="text/javascript">
    var sumas = (3+5)/2+3*0.2+12;
