                ...
                foreach ($_POST["codigocursos"] as $key => $value) {
                    ...
                    foreach ($grupos as $g) {
                        ...
                        foreach ($subgrupo as $sg) {
                            ...
                            $docentes_asignaturas_detalle->tipo = $Asignaturas->tipo;
                            $docentes_asignaturas_detalle->subgrupo = $sg;
                            $docentes_asignaturas_detalle->modalidad = 1;
                            $docentes_asignaturas_detalle->actividad = $sg;
...
//        foreach ($Modalidad as $value) {
    ...
    public function datatableRegistroAction($id, $semestre)
    {
        ...
        if ($this->request->isPOST() && $this->request->isAjax()) {
            ...
            $datatable->setSelect("semestre,grupo,docente,subgrupo,asignatura,actividad_nombre,actividad,modalidad_nombre,modalidad,numero,nro, subnro,nombre_docente,observacion,estado");
            $datatable->setFrom("( SELECT
                                            ...
                                            d_a_d.docente,
                                            d_a_d.tipo,  
                                            d_a_d.modalidad,  d_a_d.actividad,
                                            d_a_d.observacion,
                                            d_a.nro,
                                            d_a_d.subnro,
                                            m.nombres AS modalidad_nombre, ac.nombres AS actividad_nombre,
                                            m.numero AS numero,
                                            d_a_d.estado
                                            FROM
                                                    ...
                                                    INNER JOIN a_codigos AS ac ON d_a_d.actividad = ac.codigo
                                            WHERE
                                                    d.estado = 'A' and ac.numero = 55 and m.numero = 77
                                                    AND d_a_d.asignatura = '{$id}' AND d_a_d.semestre ={$semestre}
                                    ) AS temporal_table");
            $datatable->setWhere("");
            $datatable->setParams($_POST);
            $datatable->setOrderby("grupo,subgrupo, modalidad,actividad");
            $datatable->getJson();

            //AND d_a.docente = d_a_d.docente ya no va
        }
    }
