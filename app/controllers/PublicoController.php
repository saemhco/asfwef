...
class PublicoController extends ControllerPanel {

    public function initialize() {
        $this->tag->setTitle('ADMIN');
        parent::initialize();

        $this->assets->addJs("adminpanel/js/modulos/publico.js?v=" . uniqid());
    }
                            ...
                            if ($file->getKey() == "imagen") {
                                ...
                                if ($filex->getExtension() == 'jpg') {
                                    ...
                                    if (isset($Postulantes->foto)) {
                                        $url_destino = 'adminpanel/imagenes/imagenes_publico/' . $Postulantes->foto;

                                        if (file_exists($url_destino)) {
                                            unlink($url_destino);
                                        }

                                        $url_destino = 'adminpanel/imagenes/imagenes_publico/' . 'IMG' . '-' . $Postulantes->codigo . '-' . $temporal_rand . '.jpg';
                                        $Postulantes->foto = 'IMG' . '-' . $Postulantes->codigo . '-' . $temporal_rand . ".jpg";
                                    } else {
                                        $url_destino = 'adminpanel/imagenes/imagenes_publico/' . 'IMG' . '-' . $Postulantes->codigo . '.jpg';
                                    ...
                                    if (isset($Postulantes->foto)) {
                                        ...
                                        $url_destino = 'adminpanel/imagenes/imagenes_publico/' . 'IMG' . '-' . $Postulantes->codigo . '-' . $temporal_rand . '.png';
                                        $Postulantes->foto = 'IMG' . '-' . $Postulantes->codigo . '-' . $temporal_rand . ".png";
                                    } else {
                                        $url_destino = 'adminpanel/imagenes/imagenes_publico/' . 'IMG' . '-' . $Postulantes->codigo . '.png';
                                ...
                                //$file->getName() = $Postulantes->nombre;
                                //$url_destino = 'adminpanel/imagenes/imagenes_docentes/' . $Postulantes->codigo . "-" . $file->getName();
                            }
    ...
    //validar postulante en admisionregistro    
    public function admisionregistroPostulanteAction() {
        //print_r("Entro Aqui");exit();
        $this->view->disable();
        if ($this->request->isPost() && $this->request->isAjax()) {
            $postulante = AdmisionPostulantes::findFirstBypostulante((int) $this->request->getPost("postulante", "int"));


            if ($postulante) {
                //$this->response->setJsonContent($PostulantesEncuestas->toArray());
                $this->response->setJsonContent(array("say" => "yes"));
                $this->response->send();
            } else {
                //$this->response->setContent("No existe registro");
