...
class AlumnospermisosController extends ControllerPanel {

    public function initialize() {
        $this->tag->setTitle('ADMIN');
        parent::initialize();

        $this->assets->addJs("adminpanel/js/modulos/listaalumnospermisos.js?v=" . uniqid());
    }
    ...
    public function registroAction($id = null) {
        ...
        //personal_familiares
        $this->assets->addJs("adminpanel/js/modulos/alumnos.permisos.js?v=" . uniqid());
    }
    ...
    public function datatableAction() {
        ...
        if ($this->request->isPOST() && $this->request->isAjax()) {
            ...
            $datatable->setSelect("a.codigo, ca.descripcion as carrera, a.apellidop, a.apellidom, a.nombres, a.nro_doc, a.celular, a.direccion,a.cv,a.estado");
            //$datatable->setFrom("asignaturas a INNER JOIN curriculas cu ON a.curricula = cu.codigo");
            $datatable->setFrom("alumnos a
                INNER JOIN carreras ca ON a.carrera = ca.codigo
                ");
            //$datatable->setWhere("a.estado = 'A' AND a.codigo > 0");
            //$datatable->setWhere("a.estado = 'A'");
    ...
    public function datatableAlumnosPermisosAction($id) {
        $this->view->disable();
        if ($this->request->isPOST() && $this->request->isAjax()) {
            $datatable = new Datatables($this->di);
            ...
            $datatable->setWhere("personal_permisos.personal = $id AND personal_permisos.tipo = 1");
    ...
    public function saveAlumnosPermisosAction() {
    ...
    public function getAjaxAlumnosPermisosAction() {
        $this->view->disable();
        if ($this->request->isPost() && $this->request->isAjax()) {
    ...
    //eliminar
    public function eliminarAlumnosPermisosAction() {
        $this->view->disable();
        if ($this->request->isPost() && $this->request->isAjax()) {
            $obj = PersonalPermisos::findFirstBycodigo((int) $this->request->getPost("id", "int"));
            if ($obj && $obj->estado = 'A') {
