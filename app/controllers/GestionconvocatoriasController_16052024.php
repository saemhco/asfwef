...
class GestionconvocatoriasController extends ControllerPanel
{
    ...
    public function datatableValidarPagoAction() {
        ...
        if ($this->request->isPOST() && $this->request->isAjax()) {
            ...
            $datatable->setSelect("voucher,codigo, convocatoria, fecha, archivo,  estado,  nomestado,nombredocente,nro_doc,mensaje");
      ...
      //  $idvoucher=$voucherdata->voucher;
        ...
        if ($this->request->isPost() && $this->request->isAjax()) {
            try {
                        ...
                        if ($file->getKey() == "archivo") {
                            ...
                            if ($filex->getExtension() == 'pdf') {
                                ...
                                if ($voucher->save() == false) {
                                    // Cuando hay error
                                    $this->response->setStatusCode(200, "OK");
                                    $this->response->setJsonContent($voucher->getMessages().'xxxxx');
                                } else {
                                    if ($file->moveTo($url_destino)) {
                                        $this->response->setStatusCode(200, "OK");
                ...
                $this->response->setContent($ex->getMessage().'yyyy');
                $this->response->setStatusCode(500);
            }
        }
        $this->response->send();
...
SET estado=1 where voucher='{$indice}'";
    ...
    public function denegarAction() {
        ...
        if ($this->request->isPost() && $this->request->isAjax()) {
            ...
            $db_update = $this->db;
            $sql_update_a_s = " update tbl_web_convocatorias_voucher
SET estado=2,mensaje='{$mensaje}' where voucher='{$indice}'";
            $db_update->fetchOne($sql_update_a_s, Phalcon\Db::FETCH_OBJ);
