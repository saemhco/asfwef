$(document).ready(function () {
    ...
    $("#input-condicion_select").on("change", function () {
        ...
        $('#tbl_componentes1').DataTable().destroy();

        tbl_componentes1 = $("#tbl_componentes1").DataTable({
            "stateSave": true,
            "ajax": { "url": base_url + "licenciamiento1/datatableComponentes1/" + condicion_select, "type": "POST" },
            "processing": false,
            "serverSide": true,
            "order": [[1, "asc"]],
            "columns": [
            ...
            "preDrawCallback": function () {
                ...
                if (!responsiveHelper_dt_basic) {
                    responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_componentes1'), breakpointDefinition);
                }
            },
            "columnDefs": [
            ...
            initComplete: function () {
                ...
                $('div.dataTables_filter input').bind('keyup', function (e) {
                    if (e.keyCode == 13) {
                        $('#tbl_componentes1').dataTable().fnFilter(this.value);
                    }
                });
            }
        });
    ...
    tbl_componentes1 = $("#tbl_componentes1").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "licenciamiento1/datatableComponentes1", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
...
$("#input-codigo").focusout(function () {
    ...
    if (codigo === '') {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "licenciamiento1/codigoRegistradoComponentes1",
            data: { codigo: codigo },
            dataType: 'json',
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "licenciamiento1/registrocomponentes1/" + xsmart;
    } else {
...
function eliminar() {
    ...
    //
    //                                if (msg.say == "yes") {
    //                                    $('#tbl_componentes1').dataTable().fnDraw();
    //                                } else {
    //
    //                                }
    //                            }
