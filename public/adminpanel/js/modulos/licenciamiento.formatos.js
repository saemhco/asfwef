...
//alert(id);
    ...
    tbl_formatos = $("#tbl_formatos").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "licenciamiento/datatableFormatos", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
    ...
    $("#exito_formatos").dialog({
        ...
        buttons: [{
                ...
                click: function () {
                    $(this).dialog("close");
                    window.location.href = base_url + "licenciamiento/formatos";
                }
            }]
    });
...
$("#input-codigo").focusout(function () {
    ...
    if (codigo === '') {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "licenciamiento/codigoRegistradoFormatos",
            data: {codigo: codigo},
            dataType: 'json',
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "licenciamiento/registroformatos/" + xsmart;
    } else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ...
            success: {
                ...
                callback: function () {
                    $.ajax({
                        url: base_url + "licenciamiento/deletepdfFormatos",
                        type: 'POST',
                        data: {"id": xsmart},
                        success: function (msg) {

                            if (msg.say == "yes") {
                                window.location.href = base_url + "licenciamiento/registroFormatos/" + id;
                            } else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ...
            success: {
                ...
                callback: function () {
                    $.ajax({
                        url: base_url + "licenciamiento/deleteexcelFormatos",
                        type: 'POST',
                        data: {"id": xsmart},
                        success: function (msg) {
