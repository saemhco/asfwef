$(document).ready(function () {
    ...
    tbl_web_publico_ofimaticas = $("#tbl_web_publico_ofimaticas").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "gestionconvocatorias/datatableOfimaticas", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[2, "asc"], [3, "asc"]],
        "columns": [
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_web_publico_ofimaticas'), breakpointDefinition);
            }
        },
    ...
    $("#form_tbl_web_publico_ofimaticas").dialog({
        autoOpen: false,
        ...
        resizable: false,
        title: "<div class='widget-header'><h4><i class='fa fa-form'></i> Registro de Ofimaticas</h4></div>",
        show: {
            effect: "slide",
            duration: 400
        },
...
//                    if ((new Date(fecha_inicio) < new Date(fecha_actual)) && (new Date(fecha_fin) < new Date(fecha_actual))) {
                    ...
                    frmx = $("#form_tbl_web_publico_ofimaticas");
                    //var frm = new FormData(this);
                    var frm = new FormData(document.getElementById("form_tbl_web_publico_ofimaticas"));
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            ...
                            if (result.say === "yes")
                            {
                                $('#tbl_web_publico_ofimaticas').dataTable().fnDraw();
                                bootbox.alert("<strong>Se registró correctamente</strong>");
                                $("#form_tbl_web_publico_ofimaticas").dialog("close");
                            } else {
                                if (result.say === "falta") {
...
//                    }
    ...
    //Validar solo numeros ya no se usaran decimales
    /*
...
function agregar() {
    ...
    $("#form_tbl_web_publico_ofimaticas")[0].reset();
    $(".form_tbl_web_publico_ofimaticas").remove();
    $("#input-file").attr("value", "");
    $("#input-estado").prop("checked", true);
    $("#input-codigo").val("");
    //CKEDITOR.instances['input-descripcion'].setData('');
    $("#form_tbl_web_publico_ofimaticas").dialog("open");
}
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "gestionconvocatorias/getAjaxPublicoOfimaticas",
            data: {id: xsmart},
            dataType: 'json',
                ...
                $.each(response, function (i, val) {
                    ...
                    if (i === "archivo") {
                        ...
                        if (val === "" || val === null) {
                            ...
                            var valor = '<div class="alert alert-warning fade in form_tbl_web_publico_ofimaticas"><i class="fa-fw fa fa-warning"></i><strong>Pendiente</strong> Aun no ha subido un archivo.</div>';
                            $("#input-archivo").after(valor);
                        } else {
                            //
                            var valor = '<div class="alert alert-success fade in form_tbl_web_publico_ofimaticas">Click aqui para ver la imagen<a class="btn btn-ribbon" target="_BLANK" role="button" href=" ' + base_url + 'adminpanel/archivos/publico/capacitaciones/' + val + '" >  <i class="fa-fw fa fa-book"></i></a></div>';
                            $("#input-archivo").after(valor);
                        }
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "gestionconvocatorias/eliminarPublicoOfimaticas",
                            type: 'POST',
                            data: {"id": xsmart},
                            success: function (msg) {
