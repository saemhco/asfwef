$(document).ready(function () {
    ...
    tbl_postulantes = $("#tbl_postulantes").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "registropublico/datatable", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[2, "asc"], [3, "asc"]],
        "columns": [
...
//exito datos guardados
    $("#success").dialog({
        ...
        buttons: [{
                ...
                click: function () {
                    $(this).dialog("close");
                    window.location.href = base_url + "registropublico";
                }
            }]
    });
...
//Publicar form
    $("#publicar").on("click", function () {
        ...
        if (estado_registrado === "") {
            $.ajax({
                type: 'POST',
                url: base_url + "registropublico/publicoRegistrado",
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registropublico/registro/" + xsmart;
    } else {
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registropublico/eliminar",
                            type: 'POST',
                            data: {"id": xsmart},
                            success: function (msg) {
