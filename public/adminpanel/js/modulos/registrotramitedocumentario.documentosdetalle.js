$(document).ready(function () {
    ...
    tbl_documento_detalle = $("#tbl_documento_detalle").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "registrotramitedocumentario/datatableDocumentoDetalle/" + id_doc, "type": "POST" },
        "processing": false,
        "serverSide": true,
        ...
        "columns": [
            ...
            { "data": "link_drive", "name": "link_drive" },
        ...
        "drawCallback": function (oSettings) {
            ...
            if (data.archivo) {
                var archivo = "<a role='button' class='btn btn-xs btn-warning' target='_BLANK' href='" + base_url + "adminpanel/archivos/tramite_documentario/internos/" + data.archivo + "' >   <i class='fa fa-file-pdf-o' ></i></a>";
                $('td', row).eq(5).html(archivo);
            }

            if (data.link_drive) {
                var link_drive = "<a role='button' class='btn btn-xs btn-warning' target='_BLANK' href='" + data.link_drive + "'  >   <i class='fa fa-link' ></i></a>"; 
                $('td', row).eq(6).html(link_drive);               
            } 
...
function reporte_gestiontramitedocumentario_documentosdetalle_pdf() {
    ...
    if ($("#input-fecha_inicio_pdf").val() === "" || $("#input-fecha_fin_pdf").val() === "") {
        ...
        window.open(base_url + "reportes/registrotramitedocumentariodocumentosdetalle/" + id_doc + "/" + fecha_inicio + "/" + fecha_fin);
    }
}
