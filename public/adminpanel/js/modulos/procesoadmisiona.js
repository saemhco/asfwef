$(document).ready(function () {
    ...
    $("#file_inscripcionobservada_imagen").on("change", function () {
        //console.log("input-index-imagen");
        $("#save").attr("disabled", false);
        $(".errorforms").remove();
        if (this.files[0].size > 2000000) {
            //alert("Please upload file less than 2MB. Thanks!!");
            var val = '<div class="text-danger errorforms">Cargue el archivo de menos de 2 MB. ¡¡Gracias!! </div>';
            $("#input-inscripcionobservada-imagen").after(val);
            $(this).val('');
            $("#save").attr("disabled", true);
        } else {
            console.log("valida formato");
    ...
    $("#id_admision_enae").on("change", function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "procesoadmisiona/getresultados",
            data: { id_admision_enae: id_admision_enae},
            dataType: 'json',
            success: function (response) {
                if (response.say === "yes") {


                    $("#si_resultados").css("display", "block");
                    $("#no_resultados").css("display", "none");

                    $("#puntaje").text(response.puntaje);

                } else if (response.say === "no") {
         
                    $("#si_resultados").css("display", "none");
                    $("#no_resultados").css("display", "block");
                }
            }
        });
    });
...
function clickConstancia() {
    $.ajax({
        type: 'POST',
        url: base_url + "procesoadmisiona/saveContador",
    ...
    window.open(base_url + "reportesenae/reporteconstanciaenae/"+id_admision_enae);
}