$(document).ready(function () {
    ...
    tbl_convocatorias = $("#tbl_convocatorias").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "gestionconvocatorias/datatableConvocatorias", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[2, "asc"], [3, "asc"]],
        "columnDefs": [
    ...
    $("#input-file-archivo").on("change", function () {
        //console.log($("#input-file-archivo").val());
...
function postular(codigo, convocatoria_perfiles_codigo, convocatorias_titular, convocatoria_perfiles_nombre_corto, convocatoria_perfiles_nombre, convocatorias_archivo, fecha_inicio, fecha_fin) {
    ...
    if (f_a >= f_i && f_a <= f_f) {
        ...
        if (count_formacion > 0 && count_capacitaciones > 0 && count_experiencia > 0) {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "gestionconvocatorias/verificarConvocatoria",
