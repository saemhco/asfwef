$(document).ready(function () {
    ...
    tbl_indicadores = $("#tbl_indicadores").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "gestionlicenciamiento/datatableIndicadoresEvaluacion", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
        ...
        "drawCallback": function (oSettings) {
            ...
            var cumple = "";
            if (data.cumple == 1) {
                cumple = '<span class="label label-success">SI</span>';
                $('td', row).eq(4).html(cumple);
            } else if (data.cumple == 0) {
                cumple = '<span class="label label-danger">NO</span>';
                $('td', row).eq(4).html(cumple);
            } else if (data.cumple == null) {
                cumple = '<span class="label label-warning">PROCESO</span>';
                $('td', row).eq(4).html(cumple);
            }
    ...
    function carga_componentes(id_condicion, param) {
        $.post(base_url + "gestionlicenciamiento/getComponentes", { condicion: id_condicion }, function (response) {
            var html = "";
...
function cumple(id) {
    $.ajax({
        url: base_url + "gestionlicenciamiento/IndicaoresCumple",
        type: 'POST',
        data: { "id": id },
...
function observacion(id, nombre) {
    ...
    $.ajax({
        type: 'POST',
        url: base_url + "gestionlicenciamiento/getAjaxIndicadores",
        data: { id: id },
        dataType: 'json',
