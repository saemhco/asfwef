$(document).ready(function () {
    ...
    tbl_ambientesa = $("#tbl_ambientesa").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "registroambientes/datatable", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[2, "asc"], [3, "asc"]],
        "columns": [
    ...
    $("#input_id_sede").on("change", function () {

        carga_papellon($(this).val(), 0);
...
function agregar() {
    ...
    $.ajax({
        type: 'POST',
        url: base_url + "registroambientes/getNew",
        dataType: 'json',
...
function editar() {
    ...
    if ($(".selrow").is(':checked')) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "registroambientes/getAjax",
            data: { id: xsmart },
            dataType: 'json',
            success: function (response) {
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registroambientes/eliminar",
                            type: 'POST',
                            data: { "id": xsmart },
                            success: function (msg) {
...
function carga_papellon(id_sede, param) {

    $.post(base_url + "registroambientes/getPabellon", { pk: id_sede }, function (response) {
        var html = "";
        html = html + '<option value="">Seleccione...</option>';
        $.each(response, function (i, val) {
            if (param == 0) {
                html = html + '<option value="' + val.id_pabellon + '">' + val.nombre + '</option>';
            } else {
                if (val.id_pabellon == param) {

                    html = html + '<option value="' + val.id_pabellon + '" selected >' + val.nombre + '</option>';
                } else {
                    html = html + '<option value="' + val.id_pabellon + '">' + val.nombre + '</option>';
                }
            }
