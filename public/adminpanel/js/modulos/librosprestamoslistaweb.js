...
//Abrimos el modal de lectores
    ...
    tbl_prestamos = $("#tbl_prestamos").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "librosprestamoslistaweb/datatable", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[2, "asc"], [3, "asc"]],
        "columns": [
        ...
        "drawCallback": function (oSettings) {
            ...
            var fecha_reserva = res[2] + '/' + res[1] + '/' + res[0];
            //console.log(respuesta);
            $('td', row).eq(4).html(fecha_reserva);
            ...
            //console.log(respuesta);
            $('td', row).eq(5).html(fecha_entrega);
            ...
            var res = str.split("-");
            //recorremos el array por las posiciones
            var fecha_devolucion_confirmada = res[2] + '/' + res[1] + '/' + res[0];
            //console.log(respuesta);
            $('td', row).eq(6).html(fecha_devolucion_confirmada);
...
function ver(id_prestamo) {
    $.ajax({
        url: base_url + "librosprestamoslistaweb/ver",
        type: 'POST',
...
function reporte_librosprestamoslistaweb_pdf() {
    $(".errorforms").remove();
    if ($("#input-fecha_inicio_pdf").val() === "" || $("#input-fecha_fin_pdf").val() === "") {
        ...
        window.open(base_url + "reportes/reportelibrosprestamoslistaweb/" + fecha_inicio + "/" + fecha_fin);
    }
}
...
function reporte_librosprestamoslistaweb_xls() {
    $(".errorforms").remove();
    if ($("#input-fecha_inicio_xls").val() === "" || $("#input-fecha_fin_xls").val() === "") {
        ...
        fecha_fin = moment(fecha_fin1, 'DD/MM/YYYY').format('YYYY-MM-DD');
        console.log("fecha_fin:" + fecha_fin);
        window.open(base_url + "exportar/exportarlibrosprestamoslistaweb/" + fecha_inicio + "/" + fecha_fin);
    }
}
