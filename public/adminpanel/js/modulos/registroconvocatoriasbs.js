$(document).ready(function () {
    ...
    tbl_convocatorias_bs = $("#tbl_convocatorias_bs").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "registroconvocatoriasbs/datatable", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        'columnDefs': [
        ...
        "columns": [
            ...
            { "data": "texto_muestra", "name": "convbs.texto_muestra" },
            { "data": "fecha_hora", "name": "convbs.fecha_hora" },
            { "data": "id_convocatoria_bs", "name": "convbs.id_convocatoria_bs" },
            { "data": "etapa", "name": "etapas.nombres" },
        ...
        "drawCallback": function (oSettings) {
            ...
            var res_texto_muestra = texto_muestra.substring(0, 150);
            $('td', row).eq(4).html(res_texto_muestra + "...");

            var perfiles = "<a role='button' class='btn btn-xs btn-success' href='" + base_url + "registroconvocatoriasbs/perfiles/" + data.id_convocatoria_bs + "' >   <i class='fa fa-list' ></i></a>";
            $('td', row).eq(6).html(perfiles);

            var etapa = "";
            etapa = '<span class="label label-primary">' + data.etapa + '</span>';
            $('td', row).eq(7).html(etapa);
    ...
    $("#exito_convocatoriasbs").dialog({
        ...
        buttons: [{
            ...
            click: function () {
                $(this).dialog("close");
                window.location.href = base_url + "registroconvocatoriasbs";
                //location.reload();
            }
        }]
    });
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registroconvocatoriasbs/registro/" + xsmart;
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registroconvocatoriasbs/eliminar",
                            type: 'POST',
                            data: { "id": xsmart },
                            success: function (msg) {
