$(document).ready(function () {
    ...
    tbl_web_comunicados = $("#tbl_web_comunicados").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "registrocomunicados/datatable", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_web_comunicados'), breakpointDefinition);
            }
        },
        ...
        "drawCallback": function (oSettings) {
            ...
            var html = "<img src='" + base_url + "adminpanel/imagenes/comunicados/" + data.imagen + "'   width='90' height='60'  />";
...
console.log(data.fecha_hora);
        ...
        initComplete: function () {
            ...
            $('div.dataTables_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    $('#tbl_web_comunicados').dataTable().fnFilter(this.value);
                }
            });
        }
    });
    ...
    $("#exito_comunicados").dialog({
        ...
        buttons: [{
                ...
                click: function () {
                    $(this).dialog("close");
                    window.location.href = base_url + "registrocomunicados";
                }
            }]
    });
...
//Publicar form
    $("#publicar").on("click", function () {
        frmx = $("#form_tbl_web_comunicados");
        //var datos = $("#form_mantenimientos").serialize();
        //var datos = $("#form_docentes");
        var frm = new FormData(document.getElementById("form_tbl_web_comunicados"));
    ...
    //valida enter
    $("#form_tbl_web_comunicados .input").keypress(function (e) {
        if (e.which == 13) {
            $("#graba").trigger("click");
...
function agregar() {
    ...
    $("#form_tbl_web_comunicados")[0].reset();
    $("#form_tbl_web_comunicados").dialog("open");
}
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registrocomunicados/registro/" + xsmart;
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registrocomunicados/eliminar",
                            type: 'POST',
                            data: {"id": xsmart},
                            success: function (msg) {

                                if (msg.say == "yes") {
                                    $('#tbl_web_comunicados').dataTable().fnDraw();
                                } else {
