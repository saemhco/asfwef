$(document).ready(function () {
    ...
    tbl_personal_permisos = $("#tbl_personal_permisos").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "personalpermisos/datatablePersonalPermisos/" + codigo, "type": "POST"},
        "processing": false,
        "serverSide": true,
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_personal_permisos'), breakpointDefinition);
            }
        },
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            ...
                            if (result.say === "yes")
                            {
                                ...
                                $('#tbl_personal_permisos').dataTable().fnDraw();
                                bootbox.alert("<strong>Se registró correctamente</strong>");
                                $("#form_personal_permisos").dialog("close");
...
function agregar() {
    ...
    $("#form_personal_permisos")[0].reset();
    $(".form_personal_permisos").remove();
    ...
    $.ajax({
        type: 'POST',
        url: base_url + "personalpermisos/getNew",
        ...
        success: function (response) {
            ...
            $("#form_personal_permisos").dialog("open");
        }
    });
    //
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "personalpermisos/getAjaxPersonalPermisos",
            data: {id: xsmart},
            dataType: 'json',
            success: function (response) {
                ...
                $.each(response, function (i, val) {
                    ...
                    if (i === "archivo") {
                        ...
                        if (val === "" || val === null) {
                            ...
                            var valor = '<div class="alert alert-warning fade in form_personal_permisos"><i class="fa-fw fa fa-warning"></i><strong>Pendiente</strong> Aun no ha subido un archivo.</div>';
                            $("#archivo_personal_permisos_modal").after(valor);
                        } else {
                            //
                            var valor = '<div class="alert alert-success fade in form_personal_permisos">Click aqui para ver la imagen<a class="btn btn-ribbon" target="_BLANK" role="button" href=" ' + base_url + 'adminpanel/archivos/personal_permisos/' + val + '" >  <i class="fa-fw fa fa-book"></i></a></div>';
                            $("#archivo_personal_permisos_modal").after(valor);
                        }
                    }
                    ...
                    if (i === "imagen") {
                        ...
                        if (val === "" || val === null) {
                            ...
                            var valor = '<div class="alert alert-warning fade in form_personal_permisos"><i class="fa-fw fa fa-warning"></i><strong>Pendiente</strong> Aun no ha subido un imagen.</div>';
                            $("#imagen_personal_permisos_modal").after(valor);

                        } else {
                            var valor = '<div class="alert alert-success fade in form_personal_permisos">Click aqui para ver la imagen<a class="btn btn-ribbon" target="_BLANK" role="button" href=" ' + base_url + 'adminpanel/imagenes/personal_permisos/' + val + '" >  <i class="fa-fw fa fa-image"></i></a></div>';
                            $("#imagen_personal_permisos_modal").after(valor);
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "personalpermisos/eliminarPersonalPermisos",
                            type: 'POST',
                            data: {"id": xsmart},
                            success: function (msg) {
