$(document).ready(function () {
    ...
    tbl_contratos = $("#tbl_contratos").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "registropersonal/datatableContratos/"+id_personal, "type": "POST"},
        "processing": false,
        "serverSide": true,
        'columnDefs': [
            {
    ...
    $("#error_contrato_registrado").dialog({
        ...
        close: function (event, ui) {
            ...
            if (event.originalEvent) {
                window.location.href = base_url + "registropersonal/registro";
            }
            $(this).dialog('destroy');
        },
        //
    ...
    //Formulario publicar
    $("#registrar_contrato").on("click", function () {
        ...
        if (tipo === '') {
            ...
            if (id1 === '' && id2 === '') {
                $.ajax({
                    type: 'POST',
                    url: base_url + "registropersonal/numeroContrato",
    ...
    $('#archivo_resolucion').on('change', function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "registropersonal/verificapdf",
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registropersonal/contrato/" + xsmart + "/" + anio;
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registropersonal/eliminarContrato",
                            type: 'POST',
        ...
        success: function (response) {
            ...
            if (response.say === 'si') {
                ...
                if (id1 === '' && id2 === '') {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: base_url + "registropersonal/getContratos",
                        data: {anio: anio},
                        dataType: 'json',
                        success: function (response) {
                            //console.log(response.estado);
