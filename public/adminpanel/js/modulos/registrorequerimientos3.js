$(document).ready(function () {
    ...
    tbl_hdt_req_servicio = $("#tbl_hdt_req_servicio").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "registrorequerimientos3/datatable", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
    ...
    $("#success").dialog({
        ...
        buttons: [{
            ...
            click: function () {
                $(this).dialog("close");
                window.location.href = base_url + "registrorequerimientos3";
            }
        }]
    });
...
function carga_personal(id, param) {
    ...
    $.post(base_url + "registrorequerimientos3/getPersonal", { id: id }, function (response) {
        var html = "";
...
function carga_equipos(id, param) {
    ...
    $.post(base_url + "registrorequerimientos3/getEquipos", { id: id }, function (response) {
        var html = "";
        html = html + '<option value="">SELECCIONE...</option>';
        $.each(response, function (i, val) {
            //console.log(param);
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        var proceso = $('input:radio[name=selrow]:checked').attr('proceso');

        //console.log("Proceso: "+proceso);

        if (proceso === 'SOLICITUD') {
            window.location.href = base_url + "registrorequerimientos3/registro/" + xsmart;

        } else {
            bootbox.alert("<strong>Su requerimiento de servicio está siendo procesado...</strong>");
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registrorequerimientos3/eliminar",
                            type: 'POST',
                            data: { "id": xsmart },
                            success: function (msg) {
