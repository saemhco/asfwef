$(document).ready(function () {
    ...
    $("#input-condicion_select").on("change", function () {
        ...
        $('#tbl_indicadores').DataTable().destroy();

        tbl_indicadores = $("#tbl_indicadores").DataTable({
            "stateSave": true,
            "ajax": { "url": base_url + "licenciamiento/datatableIndicadores/" + componente_select, "type": "POST" },
            "processing": false,
            "serverSide": true,
            "order": [[1, "asc"]],
            "columns": [
    ...
    function carga_componentes(condicion, param) {
        ...
        $.post(base_url + "licenciamiento/getComponentes", { pk: condicion }, function (response) {
            var html = "";
            ...
            $.each(response, function (i, val) {
                if (param == 0) {
                    ...
                    html = html + '<option value="' + val.id_componente + '" selected >' + val.codigo + " - " + res_texto_muestra + "..." + '</option>';
                } else {
                    if (val.id_condicion1 == param) {
                        ...
                        html = html + '<option value="' + val.id_componente + '">' + val.codigo + " - " + res_texto_muestra + "..." + '</option>';
                    } else {
    ...
    $("#input-componente_select").on("change", function () {
        ...
        console.log("valor-componente:"+componente_select);
    ...
    tbl_indicadores = $("#tbl_indicadores").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "licenciamiento/datatableIndicadores", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
    ...
    $("#exito_formatos").dialog({
        ...
        buttons: [{
            ...
            click: function () {
                $(this).dialog("close");
                window.location.href = base_url + "licenciamiento/indicadores";
            }
        }]
    });
...
$("#input-codigo").focusout(function () {
    ...
    if (codigo === '') {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "licenciamiento/codigoRegistradoIndicadores",
            data: { codigo: codigo },
            dataType: 'json',
...
function editar() {
    ...
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "licenciamiento/registroindicadores/" + xsmart + "/" + condicion + "/" + componente;
    } else {
