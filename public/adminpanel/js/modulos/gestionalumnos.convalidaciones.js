$(document).ready(function () {
    ...
    console.log("GAAAAAAAAAAAAAAAAAA:"+id);

    tbl_notas_convalidacion = $("#tbl_notas_convalidacion").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "gestionalumnos/datatable_notas_convalidacion/" + id, "type": "POST"},
        "processing": false,
        "serverSide": true,
        "searching": true,
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_notas_convalidacion'), breakpointDefinition);
            }
        },
        ...
        initComplete: function () {
            ...
            $('div.dataTables_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    $('#tbl_notas_convalidacion').dataTable().fnFilter(this.value);
                }
            });
        }
    ...
    $("#error_tipo").dialog({
        ...
        resizable: false,
        modal: true,
        title: "<div class='widget-header text-warning'><h4><i class='fa fa-warning'></i> convalidacion </h4></div>",
        show: {
            effect: "highlight",
            duration: 300
        },
        ...
        buttons: [{
                ...
                click: function () {
                    $(this).dialog("close");
                    //window.location.href = base_url + "convalidacion/registro/" + id;
                }
            }]
    });

    //fomrulario registro
    $("#form_notas_convalidacion").dialog({
        autoOpen: false,
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            ...
                            if (result.say === "yes") {
                                ...
                                $('#tbl_notas_convalidacion').dataTable().fnDraw();
                                bootbox.alert("<strong>Se registró correctamente</strong>");


                                $("#form_notas_convalidacion").dialog("close");
...
//validacion select asignaturas
    $("#input-asignatura").on("change", function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "convalidacion/asignaturaRegistrada",
...
function agregar() {
    ...
    //$("#input-tipo").val("");
    //fin pks
    $("#form_notas_convalidacion")[0].reset();
    $("#form_notas_convalidacion").dialog("open");
...
function editar() {
    ...
    if ($(".selrow").is(':checked')) {
        ...
        if (parseInt(tipo) === 9) {
            $.ajax({
                type: 'POST',
                url: base_url + "convalidacion/getAjaxNotasConvalidacion",
        ...
        bootbox.dialog({
            ...
            buttons: {
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "convalidacion/eliminarconvalidacion",
                            type: 'POST',
                            data: {"semestre": semestre, "asignatura": asignatura, "alumno": alumno, "veces": veces, "tipo": tipo},
                            success: function (msg) {
