$(document).ready(function () {
    ...
    tbl_registrosolicitudesalumnos = $("#tbl_registrosolicitudesalumnos").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "Gestionsolicitudes/datatableSolicitudesrecibidas", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[7, "desc"]],
        "columnDefs": [
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_registrosolicitudesalumnos'), breakpointDefinition);
            }
        },
        ...
        "drawCallback": function (oSettings) {
            ...
            if (archivo !== null) {
                var archivo = "<a role='button' class='btn btn-xs btn-warning' target='_BLANK' href='" + base_url + "adminpanel/archivos/solicitudes/alumnos/FILE-RA-" + data.archivo + "' >   <i class='fa fa-file-pdf-o' ></i></a>";
...
function mensaje(semestre, alumno, numero) {
    $.ajax({
        url: base_url + "gestionsolicitudes/mensaje",
        type: 'POST',
...
function aprobar(semestre, alumno, numero) {
    bootbox.confirm("<strong>¿Esta seguro que desea aprobar?</strong>", function (result) {
        if (result === true) {
            //console.log(result);
            $.ajax({
                url: base_url + "gestionsolicitudes/aprobar",
                type: 'POST',
                data: {"semestre": semestre, "alumno": alumno, "numero": numero},
                success: function (response) {
                    if (response.say === "yes") {
                        $('#tbl_registrosolicitudesalumnos').dataTable().fnDraw();
...
function denegar(semestre, alumno, numero) {
    bootbox.confirm("<strong>¿Está seguro que desea aprobar?</strong>", function (result) {
        if (result === true) {
            //console.log(result);
            $.ajax({
                url: base_url + "gestionsolicitudes/denegar",
                type: 'POST',
                data: {"semestre": semestre, "alumno": alumno, "numero": numero},
                success: function (response) {
                    if (response.say === "yes") {
