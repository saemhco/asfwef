$(document).ready(function () {
    ...
    tbl_web_informes = $("#tbl_web_informes").DataTable({
        "stateSave": true,
        "ajax": {"url": base_url + "registroinformes/datatable", "type": "POST"},
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
        ...
        "preDrawCallback": function () {
            ...
            if (!responsiveHelper_dt_basic) {
                responsiveHelper_dt_basic = new ResponsiveDatatablesHelper($('#tbl_web_informes'), breakpointDefinition);
            }
        },
...
console.log(data.fecha_hora);
        ...
        initComplete: function () {
            ...
            $('div.dataTables_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    $('#tbl_web_informes').dataTable().fnFilter(this.value);
                }
            });
        }
    });
    ...
    $("#exito_informes").dialog({
        ...
        buttons: [{
                ...
                click: function () {
                    $(this).dialog("close");
                    window.location.href = base_url + "registroinformes";
                }
            }]
    });
...
//Publicar form
    $("#publicar").on("click", function () {
        frmx = $("#form_tbl_web_informes");
        //var datos = $("#form_mantenimientos").serialize();
        //var datos = $("#form_docentes");
        var frm = new FormData(document.getElementById("form_tbl_web_informes"));
    ...
    //valida enter
    $("#form_tbl_web_informes .input").keypress(function (e) {
        if (e.which == 13) {
            $("#graba").trigger("click");
...
function agregar() {
    ...
    $("#form_tbl_web_informes")[0].reset();
    $("#form_tbl_web_informes").dialog("open");
}
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registroinformes/registro/" + xsmart;
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registroinformes/eliminar",
                            type: 'POST',
                            data: {"id": xsmart},
                            success: function (msg) {

                                if (msg.say == "yes") {
                                    $('#tbl_web_informes').dataTable().fnDraw();
                                } else {
