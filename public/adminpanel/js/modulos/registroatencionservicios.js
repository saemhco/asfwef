$(document).ready(function () {
    ...
    tbl_atenciones = $("#tbl_atenciones").DataTable({
        "stateSave": true,
        "ajax": { "url": base_url + "registroatencionservicios/datatable", "type": "POST" },
        "processing": false,
        "serverSide": true,
        "order": [[1, "asc"]],
        "columns": [
        ...
        "drawCallback": function (oSettings) {
            ...
            var reportes = "";
            reportes = "<a role='button' title='Reporte PDF' class='btn btn-xs btn-warning' target='_BLANK' href='" + base_url + "reportes/registroatencionservicios/" + data.id_atencion + "' >   <i class='fa fa-file-pdf-o' ></i></a>\n\
            <a role='button' title='Reporte Excel' class='btn btn-xs btn-success' target='_BLANK' href='" + base_url + "exportar/registroatencionservicios/" + data.id_atencion + "' >   <i class='fa fa-file-excel-o' ></i></a>";
            $('td', row).eq(8).html(reportes);
    ...
    $("#success").dialog({
        ...
        buttons: [{
            ...
            click: function () {
                $(this).dialog("close");
                window.location.href = base_url + "registroatencionservicios";
            }
        }]
    });
        ...
        buttons: [{
            ...
            click: function () {
                ...
                if (nro_doc.length < 1 || nro_doc.length !== 8) {
                    ...
                    //console.log("Inserta");
                    $.ajax({
                        url: base_url + "registroatencionservicios/saveAtencionNuevo",
                        type: 'POST',
    ...
    $("#input-buscar").on("click", function () {
        ...
        if ($("#input-fecha_inicio").val() === "" || $("#input-fecha_fin").val() === "") {
            ...
            tbl_atenciones = $("#tbl_atenciones").DataTable({
                "stateSave": true,
                "ajax": { "url": base_url + "registroatencionservicios/datatable/" + fecha_inicio + "/" + fecha_fin, "type": "POST" },
                "processing": false,
                "serverSide": true,
                "order": [[1, "asc"]],
                "columns": [
...
function editar() {
    if ($(".selrow").is(':checked')) {
        ...
        window.location.href = base_url + "registroatencionservicios/registro/" + xsmart + "/" + nro_doc+"/" + tipo;
        ...
        bootbox.dialog({
            ...
            buttons: {
                ...
                success: {
                    ...
                    callback: function () {
                        $.ajax({
                            url: base_url + "registroatencionservicios/eliminar",
                            type: 'POST',
