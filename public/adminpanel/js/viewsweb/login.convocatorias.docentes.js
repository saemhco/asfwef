$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.say === "si") {
          ...
          $(".errorforms").remove();
          var val =
            '<div class="text-danger errorforms">El email ya está registrado</div>';
  ...
  $("#btn_grabar_postulantes").on("click", function () {
    ...
    if (input_password === input_password2) {
      ...
      var frm = new FormData(
        document.getElementById("form_registro_convocatorias")
      );
      $.ajax({
          ...
          if (result.say === "yes") {
            ...
            $("#modal_save_postulante").on("hide.bs.modal", function () {
              ...
              window.location.href =
                base_url + "login-convocatorias-docentes.html";
            });
            ...
            $(".errorforms").remove();
            var val =
              '<div class="text-danger errorforms">El campo contraseña esta vacio</div>';
            ...
            $.each(result, function (i, val) {
              ...
              } else if (i === "id_bonificacion") {
                $("#id_bonificacion").after(val);
              } else if (i === "password") {
                $("#input_password").after(val);
              }
            });
  ...
  $("#btn_login").on("click", function () {
    window.location.href = "/login-convocatorias-docentes";
  });
*/
  //
  $("#btn_login").on("click", function () {
      ...
      $.ajax({
        ...
        success: function (msg) {
          if (msg.say === "yes") {
            ...
            $("#modal_campo_vacio .modal-body").text(
            ...
            $("#modal_campo_vacio").modal("show");
            $("#modal_campo_vacio .modal-body").text(
              "Credenciales no registradas , intentelo nuevamente"
            );
            ...
            $("#modal_campo_vacio").modal("show");
            $("#modal_campo_vacio .modal-body").text(
              "Usted esta siendo evaluado en el sistema de convocatorias"
            );
  ...
  $("#input-id_bonificacion").on("change", function () {
    var id_bonificacion = $("#input-id_bonificacion").val();

    //console.log("input-id_bonificacion: "+id_bonificacion);
    ...
    if (id_bonificacion === "1") {
      ...
      $("#label_archivo").text("Adjuntar archivo de Discapacidad");

      $(".archivoFile").attr("name", "archivo_discapacitado");
    } else if (id_bonificacion === "3") {
      $("#input-archivos").attr("style", "display: block;");
      $("#label_archivo").text(
        "Adjuntar archivo de Licenciado Fuerzas Armadas"
      );

      $(".archivoFile").attr("name", "archivo_fa");
    } else if (id_bonificacion === "4") {
      $("#input-archivos").attr("style", "display: block;");
      $("#label_archivo").text("Adjuntar archivo de Deportista Calificado");

      $(".archivoFile").attr("name", "archivo_dar");
    } else if (id_bonificacion === "5") {
      $("#input-archivos").attr("style", "display: block;");
      $("#label_archivo").text("Adjuntar archivo de RENACYT");
