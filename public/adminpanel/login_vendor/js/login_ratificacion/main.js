$(function () {
  ...
  function account_ratification_docent_recoveryF() {
    var account_ratification_docent_recovery = JSON.parse(
      localStorage.getItem("account_ratification_docent_recovery")
    );
    if (account_ratification_docent_recovery != null) {
      var $field = $(".form-control").closest(".form-group");
      $field.addClass("field--not-empty");
      $("#username").val(account_ratification_docent_recovery.username);
      $("#password").val(account_ratification_docent_recovery.password);
      $("#saveAccount").prop("checked", true);
    }
  }
  account_ratification_docent_recoveryF();
  ...
  $("#saveAccount").click(function () {
    ...
    if (value) {
      localStorage.setItem(
        "account_ratification_docent_recovery",
        JSON.stringify({ username: username, password: password })
      );
    } else {
      localStorage.removeItem("account_ratification_docent_recovery");
    }
  });
      ...
      if (isLogin == false) {
        ...
        grecaptcha
          ...
          .then(function (token) {
            ...
            $.ajax({
              url: urlPath + "web/loginRatificacionDocentes",
              type: "POST",
